...
import React, { useState, useEffect } from 'react'; // MOD: removed unused useRef
...
const DataSourceScreen: React.FC = () => {
  ...
  const handleUploadToAPI = async () => {
    ...
    try {
      ...
      let backendUrl =
        'https://featurebox-ai-backend-service-666676702816.us-west1.run.app'; // Default
      ...
      if (
        ...
        backendUrl =
          'https://ladera-featurebox-ai-backend-service-666676702816.us-west1.run.app';
        ...
        backendUrl =
          'https://featurebox-ai-backend-service-666676702816.us-west1.run.app';
      ...
      if (contentType.includes('application/json')) {
        ...
        setForecastResult({ filename, downloadableFile: blob, completedAt: new Date() });
        navigate('/forecast-results');
      }
    } catch (error) {
  ...
  const resetUpload = () => setUploadError(null);
  ...
  return (
    <div className="container max-w-5xl px-4 py-12 mx-auto">
      ...
      <div className="flex justify-between items-center">
        ...
        {/* NEW: Right-side group with History + Generate Forecast */}
        <div className="flex items-center gap-2"> {/* NEW */}
          <Button
            onClick={handleForecastResults}                             // NEW
            className="flex items-center gap-1 bg-green-700 hover:bg-green-700 text-white px-3 py-2 rounded-md shadow-sm transition-colors text-sm" // NEW
            aria-label="Forecast History"                               // NEW
          >
            <BarChart3 size={16} className="mr-1" />                    {/* NEW */}
            Previous Forecasts                                                     {/* NEW */}
          </Button>
          ...
          <motion.button
            ...
            className={`btn-primary px-4 py-2 ${
              (isUploading ||
                globalForecastStatus === 'running' ||
                globalForecastStatus === 'started') &&
              'opacity-70 cursor-not-allowed'
            }`}                                                         // MOD: kept smaller padding
